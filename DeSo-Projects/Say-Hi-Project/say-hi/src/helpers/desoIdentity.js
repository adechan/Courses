class DesoIdentity {
  ...
  loginAsync(accessLevel) {
    return new Promise((resolve, reject) => {
      this.identityWindow = window.open(
        "https://identity.deso.org/log-in?testnet=true?accessLevelRequest=" +
          accessLevel,
        null,
  ...
  logout(publicKey) {
    this.identityWindow = window.open(
      "https://identity.deso.org/logout?testnet=true?publicKey=" +
        publicKey +
        "?testnet=true",
      null,
      "toolbar=no, width=800, height=1000, top=0, left=0"
  ...
  approveSignTx(transactionHex) {
    this.identityWindow = window.open(
      "https://identity.deso.org/approve?testnet=true?tx=" + transactionHex,
      null,
      "toolbar=no, width=800, height=1000, top=0, left=0"
    );
  }
